...
/// Restricts what `InteractiveContext` accepts as pickable. Installed filters gate
/// both pick-driven selection (`handlePick`) and hover (`handleHover`) — never
/// programmatic `select(_:)`, which (like `selectionMode`) is an intentional
/// override by app code. Mirrors OCCT's `SelectMgr_Filter` /
/// `StdSelect_FaceFilter` / `StdSelect_EdgeFilter` / `StdSelect_ShapeTypeFilter`
/// family, minus the C++ handle machinery.
///
...
/// (`OCCTSwift.Face.SurfaceType` — plane, cylinder, cone, sphere, torus, …). Every
/// other case (`.body`, `.edge`, `.vertex`) is accepted unconditionally; combine
/// with `ShapeTypeFilter` via `AllOfFilter` to also restrict the sub-shape kind.
...
/// `kinds` (`OCCTSwift.Edge.CurveType` — line, circle, ellipse, …). Every other
/// case is accepted unconditionally.
