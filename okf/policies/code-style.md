...
("share the link to the documentation instead of rephrasing the information"). This repo's
`docs/reference/` already documents Swift declarations per-symbol (`AreaSelection.md`,
`Dimensions.md`, `Manipulators.md`, and siblings), the same shape as OCCTSwift's own reference
pages, not CLI-verb parameter tables the way OCCTSwiftScripts' does. This rollout covers the
mechanical formatting/lint tier only (§3a of the ecosystem proposal); the docs-completeness/SSoT
drift checks proposed for that per-symbol shape (§3b) are not built here and remain later work,
tracked against the same proposal rather than invented ad hoc in this PR.
...
Why: this repo is small enough (30 files, ~6,600 lines) to sweep into full compliance in one PR
...
[#115](https://github.com/SecondMouseAU/OCCTSwiftScripts/pull/115)); this repo follows as part of
the same OCCTMCP-cluster rollout wave. Full research and rationale:
...
Filed and tracked as [OCCTSwiftAIS#42](https://github.com/SecondMouseAU/OCCTSwiftAIS/issues/42).
